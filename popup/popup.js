...
chrome.storage.local.get(["boycottZItem"]).then((result) => {
  const { country, label, type, proof, reason } = result.boycottZItem
  let msg = ``
  ...
  if (country === 'Israel' && !reason) {
    ...
    msg += `
    <br/><br/><p>
    The French government has gained notoriety for its discriminatory practices against Muslims both within and outside of France. 
    This is evident through the implementation of policies that target Islam within the country, as well as through military actions, 
